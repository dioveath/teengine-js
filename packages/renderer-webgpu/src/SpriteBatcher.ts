import { Color, Mat3, type SpriteDrawCommand } from "@teengine/core";
...
export class SpriteBatcher {
  ...
  pack(
    ...
    viewProjection: Mat3,
    textures: Map<number, { texture: GPUTexture }>,
  ): SpriteRun[] {
    ...
    for (const cmd of commands) {
      const gpu = textures.get(cmd.frame.texture.id);
      if (!gpu) throw new Error(`Unknown texture ${cmd.frame.texture.id}`);
      if (texture !== null && gpu.texture !== texture) flush();
      texture = gpu.texture;
  ...
  private addSprite(cmd: SpriteDrawCommand, viewProjection: Mat3): void {
    ...
    const corners = [
      { lx: -opts.originX * sx, ly: -opts.originY * sy, u: frame.u0, v: frame.v0 },
      { lx: (frame.width - opts.originX) * sx, ly: -opts.originY * sy, u: frame.u1, v: frame.v0 },
      { lx: (frame.width - opts.originX) * sx, ly: (frame.height - opts.originY) * sy, u: frame.u1, v: frame.v1 },
      { lx: -opts.originX * sx, ly: (frame.height - opts.originY) * sy, u: frame.u0, v: frame.v1 },
    ];
