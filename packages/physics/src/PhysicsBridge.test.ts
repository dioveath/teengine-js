...
import { createEntity, Input, Transform, World } from "@teengine/core";
...
describe("World physics pipeline", () => {
  ...
  it("runs snapshot → step → sync during fixedUpdate", () => {
    ...
    world.fixedUpdate({ dt: 1 / 60, tick: 0, time: 0, input });
  ...
  it("simulates a dynamic entity falling under gravity", () => {
    ...
    for (let i = 0; i < 30; i++) {
      world.fixedUpdate({ dt: 1 / 60, tick: i, time: i / 60, input });
    }
