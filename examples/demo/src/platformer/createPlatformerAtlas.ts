...
export const PLATFORMER_ATLAS = "platformer";
...
export function createPlatformerAtlas(graphics: Graphics): PlatformerAtlas {
  ...
  const texture = createTextureFromRgba(graphics, pixels, SIZE, CELL * ROWS);
  return {
    player: regionFromCell(texture, SIZE, CELL * ROWS, 0, 0),
    enemy: regionFromCell(texture, SIZE, CELL * ROWS, 1, 0),
    coin: regionFromCell(texture, SIZE, CELL * ROWS, 2, 0),
    uiHeart: regionFromCell(texture, SIZE, CELL * ROWS, 3, 0),
  };
}
